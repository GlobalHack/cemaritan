...
def convert_files(filename_to_contents: Dict[str, str], mapping_source: Union[str, List]):
    """Converts 9 of the HMIS standard csv files to SF objects.

    Only Client.csv is required.

    Return
    ------
    Dict of HMIS_ID to dict of the SF objects.
    """
    if isinstance(mapping_source, str):
        mapping = json.load(open(mapping_source))
    else:
    ...
    dfs = {fn: pd.read_csv(StringIO(contents)) for fn, contents in filename_to_contents.items() if fn in fns}
    # Replace nan with None for JSON serialization.
    dfs = {fn: df.where(pd.notnull(df), None) for fn, df in dfs.items()}
