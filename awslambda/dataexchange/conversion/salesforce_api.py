...
def get_auth_header_v2(creds: Dict=None, config_file: str=None):
    if creds is None:
        creds = get_credentials(config_file=config_file)
    return get_auth_header(client_id=creds['client_id'], 
...
def get_recent_records(headers: Dict=None, object_type: str='OLI_Client__c', minutes: int=5) -> List[SfRecord]:
...
def post_new_client(oli_client_obj, headers: Dict):
    """Post a new client record to SalesForce."""
    url = 'https://cs3.salesforce.com/services/data/v43.0/sobjects/OLI_Client__c'
    fields_to_exclude = ['CreatedDate']
    payload = {k:v for k,v in oli_client_obj.items() if k not in fields_to_exclude}
...
def post_new_clients(clients: List[Dict]):
    """Post list of client objects.

    Eventually need to support multiple types of objects.
    """
    results = []
    headers = get_auth_header_v2()
    total_num = len(clients)
    for i, c in enumerate(clients):
        r = post_new_client(oli_client_obj=c, headers=headers)
        results.append(r)
        print(f'{i} of {total_num} uploaded.')
    return results
